...
// import { fetchMaterials } from "@/lib/helpers"; // Assuming this helper exists
// import { Header } from "@/components/dashboard/Header";
// import { Sidebar } from "@/components/dashboard/Sidebar";
// import { MobileNav } from "@/components/dashboard/MobileNav";
// import { UploadCard } from "@/components/dashboard/UploadCard";
...
// export default function DashboardPage() {
//     // 1. Get the new `isSyncing` state from the AuthContext
//     const { user, supabase, isLoading: authLoading, isSyncing } = useSupabase();
//     const router = useRouter();
//     const queryClient = useQueryClient();
...
//     // 2. Update the redirect logic to also wait for `isSyncing`
//     useEffect(() => {
...
//     // 3. Update the `useQuery` to be enabled only when syncing is complete
//     const { data: materials, isLoading: materialsLoading, error } = useQuery({
//         queryKey: ["materials", user?.id],
//         queryFn: () => supabase.auth.getSession().then((res) => fetchMaterials(res.data.session)),
//         enabled: !!user && !authLoading && !isSyncing,
//     });
...
//     // 4. Update the main loading state to cover both auth loading and syncing
